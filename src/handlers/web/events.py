...
class EditEventPage(WebRequestHandler):
    def get(self):
        key = self['id']
        if key:
            event = Event.get(key)
            path = 'edit_event.html'
            ua = self.request.headers['User-Agent']
            b = reg_b.search(ua)
            v = reg_v.search(ua[0:4])
            template_values = {'ua' : 'non-mobile'}
            if b or v:
                template_values['ua'] = 'mobile'
            form_url = blobstore.create_upload_url('/api/events/edit_event')
            template_values['form_url'] = form_url
            template_values['is_member'] = True if 'member' in self.session else False
            if 'member' in self.session:
                template_values['member'] = Member.get_member_json(self.session['member'])
...
class SearchEventPage(WebRequestHandler):
    def get(self):
...
app = RestApplication([('/events/create_event', CreateEventPage),
